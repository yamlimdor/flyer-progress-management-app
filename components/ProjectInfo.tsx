...
const ProjectInfo: React.FC<ProjectInfoProps> = ({ project, onUpdateProject }) => {
  ...
  const handleSave = () => {
    const { eventName, eventDate, eventTime, eventLocation, printCount, deliveryHopeDate, notes, isUrgent } = editedProject;
    onUpdateProject(project.id, { eventName, eventDate, eventTime, eventLocation, printCount, deliveryHopeDate, notes, isUrgent });
    setIsEditing(false);
  };
      ...
      {isEditing ? (
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-6">
          ...
          <div>
            ...
            <input type="text" name="eventName" id="eventName" value={editedProject.eventName} onChange={handleInputChange} required className="mt-1 block w-full px-3 py-2 bg-white text-black border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#c3d825] focus:border-[#c3d825]" />
          </div>
          ...
          <div>
            ...
            <input type="text" name="eventLocation" id="eventLocation" value={editedProject.eventLocation} onChange={handleInputChange} className="mt-1 block w-full px-3 py-2 bg-white text-black border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#c3d825] focus:border-[#c3d825]" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="printCount" className="block text-sm font-medium text-gray-800">印刷枚数</label>
              <input type="number" name="printCount" id="printCount" value={editedProject.printCount} onChange={handleInputChange} required className="mt-1 block w-full px-3 py-2 bg-white text-black border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#c3d825] focus:border-[#c3d825]" />
            </div>
            <div>
              <label htmlFor="deliveryHopeDate" className="block text-sm font-medium text-gray-800">希望納品日</label>
              <input type="date" name="deliveryHopeDate" id="deliveryHopeDate" value={editedProject.deliveryHopeDate} onChange={handleInputChange} required className="mt-1 block w-full px-3 py-2 bg-white text-black border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#c3d825] focus:border-[#c3d825]" />
            </div>
          </div>
          <div>
        ...
        <>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-[max-content_1fr] gap-x-6 gap-y-4 text-lg">
            ...
            <dd className="text-black">{project.deliveryHopeDate}</dd>
          </dl>
          {project.notes && (
